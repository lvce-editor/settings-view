...
export const getSettingsDom = (state: SettingsState): readonly VirtualDomNode[] => {
  const { bottomSpacerHeight, filteredItems, height, searchValue, tabs, topSpacerHeight, totalContentHeight, visibleSections } = state
  const hasSearchValue = searchValue.trim().length > 0
  const filteredItemsCount = filteredItems.length
  return [
    parentNode,
    ...getSettingsHeaderDom(filteredItemsCount, hasSearchValue),
    ...getSettingsMainDom(tabs, visibleSections, filteredItemsCount, totalContentHeight, searchValue, height, topSpacerHeight, bottomSpacerHeight),
  ]
}
